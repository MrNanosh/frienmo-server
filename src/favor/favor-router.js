...
favorRouter.route("/").get(async (req, res) => {
  ...
  let favors = await FavorService.getAllFavors(req.app.get("db"), limit, page);

  return res.status(200).json({ favors, page, limit });
});
favorRouter
  .use(requireAuth)
  .route("/issue/:favor_id")
  .patch(jsonBodyParser, async (req, res, next) => {
    let { receiver_id } = req.body;
    const receiver = { receiver_id };
    FavorService.updateOutstanding(
      req.app.get("db"),
      req.params.favor_id,
      req.user.id,
      receiver
...
//   let outstanding = await FavorService.getOutstanding(db, favor_id);
//   outstanding.id = outstanding_id;
//   outstanding.receiver_id = receiver_id;
//   console.log("id", id);
//   let checked = false;
//   console.log("if o/s rec is null", receiver_id);
//   if (!outstanding.receiver_id) {
...
//     );
//     checked = true;
//     console.log("if o/s rec is null", checked);
//     return res.status(201).send();
//   }

//   if (!checked) {
//     let favor = await FavorService.getFavorById(db, favor_id);
//     if (outstanding.length < (favor.limit || 1)) {
//       //allow issuing of favor
//       await FavorService.insertOutstanding(db, {
//         favor_id,
//         users_id,
//         receiver_id,
...
favorRouter
  ...
  .get("/personal", async (req, res) => {
    ...
    if (!user_id) {
      return res.status(404).json("must have an authorized account to use");
    }
    if (!limit) {
      limit = 30;
    }
    ...
    let favors = await FavorService.getPersonalFavors(db, user_id, limit, page);
    return res.status(200).json({ favors, page, limit });
  })
  .get("/friend", async (req, res) => {
    ...
    let favors = await FavorService.getFavorByFriends(db, user_id, limit, page);
    return res.status(200).json({ favors, page, limit });
  })
  .get("/public", async (req, res) => {
    ...
    let favors = await FavorService.getPublicFavors(db, user_id, limit, page);
    return res.status(200).json({ favors, page, limit });
  })
  .patch("/redeem/:favor_id", jsonBodyParser, async (req, res) => {
    ...
    //favor must exist
    let ticket = await FavorService.getOutstanding(db, req.params.favor_id)[0];
    if (!ticket) {
    ...
    if (!req.user) {
      return res.status(401).json({
        error: "you must be logged in to redeem a favor"
      });
    } else {
    ...
    let redeemedTicket;
    if (ticket.giver_redeemed === true) {
      res.status(403).json({
        error: "this favor has already been redeemed"
      });
    }

    if (ticket.users_id === person) {
      if (ticket.receiver_redeemed === false) {
        return res.status(403).json({
          error: "this user may not yet confirm the favor"
        });
      }
      redeemedTicket = await FavorService.redeem(db, outstanding_id, {
        giver_redeemed: true
      });
    }
    if (ticket.receiver_id === person) {
      redeemedTicket = await FavorService.redeem(db, outstanding_id, {
  ...
  .get(async (req, res) => {
    ...
    if (favor.publicity !== "public") {
      ...
      if (
        ...
        authuser !== favor.receiver_id ||
        authuser !== favor.creator_id
      ) {
        if (favor.publicity === "friends") {
          const issuerFriends = FriendService.getFriends(db, favor.issuer_id);
          const receiverFriends = FriendService.getFriends(
            db,
            favor.receiver_id
          );
          const creatorFriends = FriendService.getFriends(db, favor.creator_id);
          const hasIssuerAsFriend = (await issuerFriends).reduce(
            (acc, curr) => (curr.id === authuser ? true : acc),
            false
          );
          const hasReceiverAsFriend = (await receiverFriends).reduce(
            (acc, curr) => (curr.id === authuser ? true : acc),
            false
          );
          const hasCreatorAsFriend = (await creatorFriends).reduce(
            (acc, curr) => (curr.id === authuser ? true : acc),
            false
          );
          if (
            ...
            return res.status(403).json({
              error: "unable to use access this favor"
            });
          }
        } else {
          return res.status(403).json({
  ...
  .patch(jsonBodyParser, async (req, res) => {
    ...
    if (limit) {
      if (limit < outstanding.limit) {
        return res.status(400).json({
          error: "can not decrease limit below outstanding favors"
        });
      }
    }
    ...
    return res.status(201).json(updatedFavor);
  })
  .delete(async (req, res) => {
    await FavorService.deleteFavor(req.app.get("db"), req.params.id);
